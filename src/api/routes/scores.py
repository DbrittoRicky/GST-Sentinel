...
from src.api.database import get_conn, release_conn          # ← added release_conn
...
async def get_scores(date: str = Query(..., description="Date in YYYY-MM-DD format")):
    ...
    try:
        cur = conn.cursor()
        cur.execute("SELECT zone_id, z_score FROM score_cache WHERE date = %s", (date,))
        rows = cur.fetchall()
        cur.close()
        if rows:
            scores = {r[0]: r[1] for r in rows}
            return JSONResponse(content={"date": date, "scores": scores,
                                          "top_zones": get_top_zones(scores), "source": "cache"})
    except Exception as e:
        print(f"DB cache miss: {e}")
    finally:
        release_conn(conn)                                    # ← always return to pool
    ...
    return JSONResponse(content={"date": date, "scores": scores,
                                  "top_zones": top_zones, "source": "gnn_csv"})
